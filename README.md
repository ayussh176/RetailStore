...
git clone https://github.com/ayussh176/RetailStore.git
cd RetailStore
...
Connect with MySQL and run the SQL scripts (see below) to create all required tables: `products`, `customers`, `suppliers`, `retailers`, `orders`, `order_items`, `cart`.
...
The backend will run on `http://localhost:5000` (or your `.env` port).

**Start Frontend:**
npm run dev

Frontend runs on [http://localhost:8080](http://localhost:5173) by default.
